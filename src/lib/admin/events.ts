...
const EVENT_SELECT = `
  ...
  starts_at, timezone, location_ar, registration_open,
  youtube_live_url, facebook_live_url,
...
export async function adminCreateEvent(input: EventInput): Promise<string> {
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      youtube_live_url: input.youtube_live_url ?? null,
      facebook_live_url: input.facebook_live_url ?? null,
